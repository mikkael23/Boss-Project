...
namespace authguard
{
    public class API
    {
        ...
        public void init()
        {
            try
            {
                ...
                if (!decoded_response.success)
                    MessageBox.Show(decoded_response.message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ...
                switch (response_data[0])
                {
                    case "started_program":
                        if (response_data[3] == "0" && response_data[4].Length > 0)
                        {
                            ...
                            {
                                MessageBox.Show("File has been tampered with, couldn't verify integrity!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    ...
                    case "update_available":
                        ...
                        else MessageBox.Show("Please add a valid download link!", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        Process.GetCurrentProcess().Kill();
                        break;
            ...
            {
                MessageBox.Show("Invalid Encryption key!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Process.GetCurrentProcess().Kill();
            }
        }
        public bool login(string username, string password, string hwid = null)
        {
            ...
            if (!is_initialized)
            {
                MessageBox.Show("Please initialize your application first!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
        ...
        public string variable(string name)
        {
            if (!is_initialized)
            {
                MessageBox.Show("The program wasn't initialized!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return "not_initialized";
            }

            if (!logged_in)
            {
                MessageBox.Show("You can only grab server sided variables after being logged in!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private string do_request(string type, NameValueCollection post_data)
        {
            ...
            {
                MessageBox.Show("Connection failure. Please try again, or contact us for help.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            return null;
        }
        public void errCheck(string data)
        {
            ...
            {
                MessageBox.Show("Possible malicious activity detected!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Process.GetCurrentProcess().Kill();
            }
            switch (data)
            {
                case "program_doesnt_exist":
                    MessageBox.Show("The program key you tried to use doesn't exist!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Process.GetCurrentProcess().Kill();
                    break;
                case "program_disabled":
                    MessageBox.Show("Looks like this application is disabled, please try again later!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Process.GetCurrentProcess().Kill();
                    break;
                case "program_banned":
                    MessageBox.Show("This application has been banned for violating the TOS" + Environment.NewLine + "Contact us at support@authguard.net", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Process.GetCurrentProcess().Kill();
                    break;
                case null:
                    Process.GetCurrentProcess().Kill();
    ...
    public static class security
    {
        ...
        public static void Start()
        {
            ...
            if (started)
            {
                MessageBox.Show("A session has already been started, please end the previous one!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Process.GetCurrentProcess().Kill();
            }
            else
            {
                using (StreamReader sr = new StreamReader($@"{drive}Windows\System32\drivers\etc\hosts"))
                {
                    ...
                    {
                        breached = true;
                        MessageBox.Show("DNS redirecting has been detected!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        Process.GetCurrentProcess().Kill();
                    }
                }
                started = true;
        ...
        public static void End()
        {
            if (!started)
            {
                MessageBox.Show("No session has been started, closing for security reasons!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Process.GetCurrentProcess().Kill();
            }
            else
            {
